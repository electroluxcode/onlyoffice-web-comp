...
多实例打开文档使用 `onlyOfficeManagerFactory.open()`，见 [02 - 核心 API](./02-核心API.md#onlyofficemanagerfactory)。

**注意**：多实例下必须用 `data-onlyoffice-container-id` 精确定位容器，避免图片上传等操作路由到错误实例。

## 底层方式（高级）

需要直接控制 `EditorManager` 时，可使用 `createEditorView` 或 `editorManagerFactory`。详见 [02 - 核心 API](./02-核心API.md#底层-api)。
